...
**2026-08-10 — Hallazgo real durante la validación de ADR-010 (Kit → Bundle):** `fn_check_resource_availability` estaba completamente documentada en este ADR, con su SQL completo, pero **nunca se había incluido en `core-schema.sql`** — un hueco entre documentación y esquema aplicado que la revisión estática original no detectó (se revisaba el ADR y el esquema por separado). Corregida añadiéndola como migración de Core (`00005_fn_check_resource_availability.sql`), con el mismo código exacto que ya documentaba este ADR, sin ningún cambio de lógica. Validada indirectamente por `kit-bundle-resolution-tests.sql` (TEST 2: resuelve correctamente disponibilidad en lote, distinguiendo un recurso disponible de otro en reparación). Sigue pendiente un test que ejercite directamente `conflicting_event_id` con dos eventos reales solapando — se añadirá si aparece un caso de uso que lo necesite (ej. UI que muestre "ocupado por [evento]" antes de reservar).
