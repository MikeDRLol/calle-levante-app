...
6. **Reglas de negocio que varían (liquidaciones, checklist, permisos) se modelan como datos en tablas, no como funciones con valores hardcodeados.** No hace falta un motor de reglas genérico con UI todavía — sí hace falta que el número no esté clavado en TypeScript. Ver discusión en ADR-006 y ADR-007 (`amount_owed` de Settlements es dato editable, no una fórmula en código — aunque en v1 se introduce a mano, ver deuda documentada en ADR-007).
...
- **Settlements diseñado y validado en ejecución real** (2026-08-10): `docs/architecture/settlements-schema.sql` — `event_expenses`, `event_settlement_participants`, `settlement_transfers` y `fn_calculate_event_settlement` (algoritmo voraz de mínimas transferencias). Ver ADR-007. **Deuda conocida y documentada**: `amount_owed` por participante es entrada manual en v1 — la fórmula real de reparto vive en el Google Sheets original y no se ha adivinado aquí. Falta la UI (`/liquidaciones`, `/bizums`, hoy placeholders).
- **Siguiente paso:** UI de Settlements, o cerrar la deuda de Documents (Core según ADR-005, sin tabla propia todavía) — ver `docs/architecture/README.md` para el estado completo de módulos.
