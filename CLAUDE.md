...
6. **Reglas de negocio que varían (liquidaciones, checklist, permisos) se modelan como datos en tablas, no como funciones con valores hardcodeados.** No hace falta un motor de reglas genérico con UI todavía — sí hace falta que el número no esté clavado en TypeScript. Ver discusión en ADR-006. En Settlements (ADR-007) esto se llevó hasta el final: se confirmó con el negocio que Montaje e Incentivo comercial no tienen fórmula fija (varían por evento), así que `amount_owed` manual **es** el dato correcto — no lo sustituyas por una fórmula inventada.
...
- **Kit → Bundle resuelto y validado** (2026-08-10): `fn_apply_kit_to_event` (ADR-010) aplica un Kit a un evento, resolviendo cada línea a instancias reales disponibles (reutilizando `fn_check_resource_availability` en lote, nunca candidato a candidato) y generando checklist automáticamente. Esta validación encontró y corrigió 3 gaps reales entre documentación y esquema aplicado — ver Registro de validación de ADR-003 y ADR-010. En particular: **si vuelves a tocar cualquier ADR que documente SQL de una función, verifica que esa función esté realmente en un fichero de `supabase/migrations/`** — `fn_check_resource_availability` llevaba desde ADR-003 documentada pero nunca aplicada, y nadie lo detectó hasta que un módulo posterior (Kit→Bundle) intentó usarla de verdad.
- **Settlements completo (esquema + UI) y validado**: `docs/architecture/settlements-schema.sql` — `event_expenses`, `event_settlement_participants`, `settlement_transfers`, `fn_calculate_event_settlement`. Ver ADR-007. `amount_owed` manual es el diseño final y confirmado (no hay fórmula fija de Montaje/Incentivo comercial que automatizar), no una deuda pendiente.
- **Módulos Personal, Calendario, Facturas (registro de cobros, no fiscal) y Configuración construidos** — ya no queda ningún placeholder en la navegación.
- **Siguiente paso:** cerrar la deuda de Documents (Core según ADR-005, sin tabla propia todavía), o CRM completo (`clients` sigue en versión mínima, ADR-009) — ver `docs/architecture/README.md` para el estado completo de módulos.
